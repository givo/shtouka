...
Build a side-scrolling music game in React where a black dog named Sol runs through 10 themed zones, each with its own song, visual style, and obstacles. The player clicks/taps to make Sol jump over obstacles. The game progresses through increasingly chill vibes, ending with a romantic finale and personal message.
...
- **Black dog** Black Golden Retriever (similar to the previous SVG design)
...
**Theme**: Boy band meets bubblegum pop - synchronized dance energy with cheerful, radio-friendly vibes

**⚠️ CRITICAL: 35-SECOND ZONE LIMIT**
Each zone MUST complete within 35 seconds maximum, regardless of song length. This ensures the player experiences all 10 songs in a reasonable timeframe (~6 minutes total). Implementation:
...
- **Background**: Stage with gradient from hot pink to electric blue, colored spotlights rotating
- **Ground**: Candy-striped stage floor (pink/white) with white light strips
- **Color Palette**: Neon pinks, electric blue, lime green, yellow, disco ball reflections
- **Animated Elements**:
  - Disco ball spinning in background
  - Floating bubbles and sparkles appearing
...
2. **Giant Lollipop** (spiral pattern, pink/white)
3. **Star** (5-pointed, yellow, spinning)
4. **Fan Poster** ("WE ♥ SOL" with hearts)
...
- **Album Download Button**:
  ...
  - **Action**: Downloads ZIP file with all 10 MP3 songs
- **Play Again Button**: Below album button, simpler style
- **Song Selector**: Grid or list of all 10 zones for individual replay
...
Every zone MUST complete within 35 seconds to ensure the entire game (10 zones) takes approximately **6 minutes**.
...
- 10 zones × 35 seconds = **350 seconds = 5 minutes 50 seconds**
- Plus transitions (1s each × 10) = **~6 minutes total**
...
1. **Preload all 10 songs** on game start
...
- **Zone 1 (Pop Party)**: Medium - obstacles every 2.5 seconds
- **Zone 2 (Cartoon)**: Medium-Easy - obstacles every 2.5-3 seconds
- **Zone 3 (Israeli Rock)**: Medium-Hard - obstacles every 2.5 seconds
- **Zone 4 (Heavy Rock)**: Hard - obstacles every 2 seconds, fast scroll
- **Zone 5 (Ending Song)**: Easy - collectibles only, no death
- **Zone 6 (Hawaiian)**: Medium - obstacles every 3 seconds
- **Zone 7 (Reggae)**: Easy-Medium - obstacles every 3-3.5 seconds
- **Zone 8 (Hawaiian Acoustic)**: Easy - obstacles every 3.5 seconds
- **Zone 9 (Eastern)**: Medium - obstacles every 3 seconds
- **Zone 10 (Finale)**: Easy - collectibles only, celebration!
...
- Zones 5 and 10 cannot lose lives (safe zones)
...
- Total game: **~6 minutes** (including all transitions)
...
- **Preload All Assets**: Load all 10 songs before game starts (show progress)
...
- Implement all 10 zone configurations
...
- **Create ZIP file with all 10 songs**
- Loading screens
- Error handling
...
All 10 MP3 files listed in zone configurations:

1. `קוראים לו שטוקה טוקה פופ.mp3`
2. `שטוקה טוקה - קצבי, תצוגרות.mp3`
3. `בוקר הוא קופץ עלי - רוק מזרחי ישראלי.mp3`
4. `קוראים לו שטוקה טוקה - רוק כבד.mp3`
5. `קוראים לו שטוקה טוקה - שיר סיום.mp3`
6. `שטוקה טוקה - הוואי מלא.mp3`
7. `שטוקה טוקה- רגאי.mp3`
8. `שטוקה טוקה - הוואי.mp3`
9. `בוקר הוא קופץ עלי - חוסי מזרחי.mp3`
10. `קוראים לו שטוקה טוקה תצוגרות.mp3`

**PLUS**: Pre-packaged ZIP file containing all 10 songs for the download button:
...
- Background: `linear-gradient(#FF1493, #00BFFF)` with stage spotlights
- Ground: `#FFB6C1` with `#FFFFFF` stripes and light strips
- Accents: `#FF00FF`, `#00FFFF`, `#00FF00`, `#FFFF00`
...
const unlockAudio = () => {
  ...
  silentAudio.src =
    "data:audio/mp3;base64,";
...
const loadProgress = () => {
  ...
  const hoursSinceLastPlay =
    (Date.now() - gameState.timestamp) / 1000 / 60 / 60;
...
- [ ] All 10 songs play and transition smoothly
...
- [ ] All 10 songs preload before game starts
...
- [ ] ZIP file contains all 10 MP3 files
...
2. **Add the 10 MP3 files** to `/public/audio/` directory
...
7. **Build all 10 zones** one by one
...
- 10 MP3 files: ~40-55MB (depends on quality)
- Code + Assets: ~5-10MB
